...
namespace dwm_symbol {
	...
	DebugInfo* GetModuleDebugInfo(HMODULE module) {
		...
		auto pDebug = (IMAGE_DEBUG_DIRECTORY*)(pNt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress + (CHAR*)module);
	...
	bool init(void* pdbFile_baseAddress) {
		...
		constexpr size_t hook_symbol_hash = 0xbd0b33ff449e5920;
		...
		//std::ofstream file("d3d11syms.txt", std::ios_base::out);
		...
		for (const PDB::HashRecord& hashRecord : hashRecords)
		{
			...
			{
				if (strhash(record->data.S_PUB32.name) == hook_symbol_hash) {
					hook_offset = rva;
					break;
				}
				//file << record->data.S_PUB32.name << "\n";
			}
		}
		...
		for (const PDB::ModuleInfoStream::Module& module : modules) {
			...
			moduleSymbolStream.ForEachSymbol([hook_symbol_hash,/*&file,*/&strhash,&imageSectionStream](const PDB::CodeView::DBI::Record* record) {
				...
				{
					//file << name << "\n";
					if (strhash(name) == hook_symbol_hash) {
						hook_offset = rva;
					}
				}
