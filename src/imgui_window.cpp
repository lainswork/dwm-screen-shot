...
namespace imgui_window {
    ...
    bool init() {
        wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, _T("DWM 截图工具"), NULL };
        ::RegisterClassEx(&wc);
        hwnd = ::CreateWindow(wc.lpszClassName, _T("注入dwm.exe完成截图"), WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc.hInstance, NULL);
